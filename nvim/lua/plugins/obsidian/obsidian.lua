-- Obsidian plugin spec; imported through plugins/obsidian.lua.
...
local calendar_events_enabled = true -- Set false to disable Obsidian Calendar Events.
wk.add({
...
local function setup_daily_mentions()
  vim.api.nvim_create_autocmd("BufWritePost", {
    ...
    callback = function(args)
      ...
      if added then
        require("plugins.obsidian.obsidian_calendar").refresh_year(year)
      end
    end,
  })
end
...
local function attach_calendar_mappings(bufnr)
  local calendar = require("plugins.obsidian.obsidian_calendar")
...
return {
  ...
  keys = {
    ...
    { "<leader>od", "<cmd>Obsidian today<cr>", desc = "Open daily note" },
    {
      "<leader>oC",
      function()
        require("plugins.obsidian.obsidian_calendar").open_current()
      end,
  ...
  config = function(_, opts)
    ...
    require("plugins.obsidian.obsidian_calendar_events").setup({
      enabled = calendar_events_enabled,
      daily_date = calendar.daily_date,
      calendar_year = calendar.calendar_year,
      on_update = calendar.refresh_existing_year,
    })
